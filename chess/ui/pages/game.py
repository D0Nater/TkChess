...
class GamePage(BasePage):
    ...
    def __init__(self, parent: Any, controller: "BaseWindow") -> None:
        ...
        self._buttons_map = self._generate_map()

        self._step_color = FigureColor.WHITE
        self._is_figure_selected = False
        self._steps: list[list[int]] = []
        self._selected_coords: list[int] | None = None
    ...
    def _update_buttons_map(self, grid: bool = False) -> None:
        for row_idx in range(len(self._buttons_map)):
            for column_idx, figure in enumerate(self._game_map.figures_map[row_idx]):
                self._buttons_map[row_idx][column_idx].configure(
                ...
                if grid:
                    self._buttons_map[row_idx][column_idx].grid(row=row_idx, column=column_idx, ipadx=0, ipady=0)

    def _click_field(self, selected_row: int, selected_column: int) -> None:
        selected_btn = self._buttons_map[selected_row][selected_column]
        ...
        if selected_btn["bg"] in {CellColor.STEP_CELL, CellColor.KILL_STEP_CELL}:
            before_r, before_c = self._selected_coords  # type: ignore[misc]
            before_btn = self._buttons_map[before_r][before_c]
            before_figure = self._game_map.figures_map[before_r][before_c]

            selected_btn.configure(image=before_btn["image"], width=68, height=70)
            ...
            return None
        elif selected_figure is None or selected_coords == self._selected_coords:
            if self._is_figure_selected:
            ...
            return None
        elif self._is_figure_selected:
            self._set_default_colors()

        self._is_figure_selected = True
        ...
        for r, c in steps:
            if 0 <= r <= 7 and 0 <= c <= 7:
                ...
                btn_step = self._buttons_map[r][c]
                ...
                self._steps.append([r, c])

    def _set_default_colors(self) -> None:
        for row, column in self._steps:
            btn = self._buttons_map[row][column]
